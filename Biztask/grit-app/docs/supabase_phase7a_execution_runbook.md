...
### 완료: public schema 1~7번

대웅 + 클로드가 Supabase MCP로 적용 완료했다.

- `current_user_tier_rank()` helper 함수 작동 확인.
- `profiles/posts/comments/reactions/follows` RLS 활성화 5/5 확인.
- public schema 정책 25개 확인.
- `profiles_prevent_privilege_escalation` trigger 확인.
- `reactions_count_trigger`, `comments_count_trigger` 확인.
- `reactions_one_per_target_per_user` unique index 확인.

### 결정: storage schema 8번은 V1 정책 재사용

`storage.objects`는 Supabase MCP와 Dashboard SQL Editor 모두 `must be owner of table objects` 권한 문제로 `ALTER/DROP/CREATE POLICY` 실행이 불가했다.

현재 상태:

- V1 `008_storage_buckets` 정책 8개가 이미 존재한다.
- `avatars`, `post-images` buckets 둘 다 `public=true` 확인 완료.
- avatars/post_images 각각 SELECT/INSERT/UPDATE/DELETE 정책 존재.
- 기존 storage 정책은 authenticated 체크와 owner path 체크는 있지만 `current_user_tier_rank() >= 1` 강화는 없다.

영향:

- 클라이언트에서는 `useTier`와 `useImageUpload`가 익명 업로드를 차단하므로 UX 레벨은 안전하다.
- 출시 전 Storage Policies UI에서 service role 권한으로 재시도하거나, tier 검사 RPC wrapper를 만들어 서버 레벨 강화를 완료한다.
...
현재는 위 “적용 결과” 기준으로 public schema 1~7번이 원격 DB에 적용되어 있다. storage 8번은 V1 정책 재사용으로 결정했다.
...
3. 2026-04-29 기준 public schema 1~7번은 이미 적용 완료. 재실행이 필요하면 storage 8번은 실행하지 않는다.
4. 성공 후 아래 “적용 후 검증 SQL”을 순서대로 실행.

주의:

- 현재 migration 파일의 storage 8번은 권한 이슈로 실행 대상에서 제외되어 있다.
...
주의: 2026-04-29 실행 때 storage 8번은 Dashboard SQL Editor에서도 owner 권한 부족으로 실패했다. 현재 repo의 migration 파일은 storage DDL을 실행하지 않도록 메모 처리되어 있다.
...
참고: 실제 원격 DB의 storage 정책명은 V1 008 정책명을 따를 수 있다. 핵심은 avatars/post-images 각각 SELECT/INSERT/UPDATE/DELETE 정책 4개씩, 총 8개가 존재하는지다.
...
**storage 부분 결정 (2026-04-29)**: V1 008 정책 재사용. Supabase MCP/Dashboard SQL Editor 모두 `storage.objects` ALTER 권한 부족. Storage Policies UI에서 service_role 사용하거나 출시 전 RPC wrapper 추가 예정.
...
- `must be owner of table objects` 또는 `permission denied for schema storage`
  - storage 8번은 실행하지 않는다. V1 008 정책 재사용 상태로 두고, 출시 전 Storage Policies UI/RPC wrapper로 별도 처리한다.
...
- public 정책 확인: 25개
- storage 정책 확인: V1 008 정책 8개 재사용, V2 tier 강화는 출시 전 처리
