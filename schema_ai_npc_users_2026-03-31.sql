-- ═══════════════════════════════════════════════════════
-- [AI NPC 3차 기동부대] 독립 auth.users + profiles + personas 생성
-- 날짜: 2026-03-31
-- 용도: 8명의 AI 특화 NPC를 각각 독립 계정으로 생성
--       (기존 schema_personas_ai_wave3는 vip_uid 공유 → 폐기)
--
-- 중요: 각 NPC가 독립적인 auth.users 엔트리를 가져야
--       npc-cron이 해당 user_id로 게시글/댓글을 작성할 수 있음
--
...
DO $$
DECLARE
  uid_heavy     UUID := gen_random_uuid();
  uid_insight   UUID := gen_random_uuid();
  uid_complain  UUID := gen_random_uuid();
  uid_agimanse  UUID := gen_random_uuid();
  uid_ddbrother UUID := gen_random_uuid();
  uid_aiboss    UUID := gen_random_uuid();
  uid_promptplz UUID := gen_random_uuid();
  uid_easyplz   UUID := gen_random_uuid();
  now_ts TIMESTAMPTZ := now();
BEGIN

  -- ═══════════════════════════════════════════════════════
  -- STEP 1: auth.users 8명 생성 (각각 독립 UUID)
  -- ═══════════════════════════════════════════════════════
  ...
  VALUES
    (uid_heavy,     '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'npc_heavy_uploader@biztask.bot',  crypt('npc_bot_pass_' || uid_heavy::text, gen_salt('bf')),  now_ts, now_ts, now_ts, '', ''),
    (uid_insight,   '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'npc_insight_dealer@biztask.bot',  crypt('npc_bot_pass_' || uid_insight::text, gen_salt('bf')),  now_ts, now_ts, now_ts, '', ''),
    (uid_complain,  '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'npc_pro_complainer@biztask.bot',  crypt('npc_bot_pass_' || uid_complain::text, gen_salt('bf')),  now_ts, now_ts, now_ts, '', ''),
    (uid_agimanse,  '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'npc_agi_manse@biztask.bot',       crypt('npc_bot_pass_' || uid_agimanse::text, gen_salt('bf')),  now_ts, now_ts, now_ts, '', ''),
    (uid_ddbrother, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'npc_dd_brothers@biztask.bot',     crypt('npc_bot_pass_' || uid_ddbrother::text, gen_salt('bf')), now_ts, now_ts, now_ts, '', ''),
    (uid_aiboss,    '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'npc_ai_curious_boss@biztask.bot', crypt('npc_bot_pass_' || uid_aiboss::text, gen_salt('bf')),    now_ts, now_ts, now_ts, '', ''),
    (uid_promptplz, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'npc_prompt_please@biztask.bot',   crypt('npc_bot_pass_' || uid_promptplz::text, gen_salt('bf')), now_ts, now_ts, now_ts, '', ''),
    (uid_easyplz,   '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'npc_easy_explain@biztask.bot',    crypt('npc_bot_pass_' || uid_easyplz::text, gen_salt('bf')),   now_ts, now_ts, now_ts, '', '')
  ON CONFLICT (id) DO NOTHING;

  -- ═══════════════════════════════════════════════════════
  -- STEP 2: profiles 8명 생성 (닉네임 + 아바타)
  -- ═══════════════════════════════════════════════════════
  INSERT INTO public.profiles (id, nickname, avatar_url, is_vip)
  VALUES
    (uid_heavy,     '헤비업로더',     NULL, false),
    (uid_insight,   '인사이트호소인', NULL, false),
    (uid_complain,  '프로불편러',     NULL, false),
    (uid_agimanse,  'AGI만세',       NULL, false),
    (uid_ddbrother, 'ㄷㄷ형님들',     NULL, false),
    (uid_aiboss,    'AI궁금한사장',   NULL, false),
    (uid_promptplz, '프롬프트좀요',   NULL, false),
    (uid_easyplz,   '쉽게설명좀',     NULL, false)
  ON CONFLICT (id) DO NOTHING;

  -- ═══════════════════════════════════════════════════════
  -- STEP 3: personas 8명 생성 (각각 독립 user_id)
  -- ═══════════════════════════════════════════════════════
  ...
  INSERT INTO public.personas (
    ...
    uid_heavy, '헤비업로더', NULL, 'IT/개발', '가벼운',
...
- AI 찬양이나 비관론. 중립적 정보 전달만',
  ...
  INSERT INTO public.personas (
    ...
    uid_insight, '인사이트호소인', NULL, 'IT/컨설팅', '전문적',
...
- 상대방 인격 공격. 논리로만 반박',
  ...
  INSERT INTO public.personas (
    ...
    uid_complain, '프로불편러', NULL, 'IT/보안', '냉소적',
...
- 단답형. 왜 부정적인지 근거를 항상 씀',
  ...
  INSERT INTO public.personas (
    ...
    uid_agimanse, 'AGI만세', NULL, 'IT/스타트업', '열정적',
...
- 인격 공격',
  ...
  INSERT INTO public.personas (
    ...
    uid_ddbrother, 'ㄷㄷ형님들', NULL, 'IT/일반', '순종적',
...
- 너무 긴 댓글',
  ...
  INSERT INTO public.personas (
    ...
    uid_aiboss, 'AI궁금한사장', NULL, '자영업/요식업', '호기심',
...
- 부정적 딴지 (긍정적으로 궁금해하는 스타일)',
  ...
  INSERT INTO public.personas (
    ...
    uid_promptplz, '프롬프트좀요', NULL, '마케팅', '적극적',
...
- 무관한 주제에 댓글',
  ...
  INSERT INTO public.personas (
    ...
    uid_easyplz, '쉽게설명좀', NULL, '일반/사무직', '순수한',
...
- 부정적 딴지 (그냥 궁금하기만 함)',
  ...
  RAISE NOTICE '✅ AI NPC 8명 생성 완료! (auth.users + profiles + personas)';
  RAISE NOTICE '  - 헤비업로더: %', uid_heavy;
  RAISE NOTICE '  - 인사이트호소인: %', uid_insight;
  RAISE NOTICE '  - 프로불편러: %', uid_complain;
  RAISE NOTICE '  - AGI만세: %', uid_agimanse;
  RAISE NOTICE '  - ㄷㄷ형님들: %', uid_ddbrother;
  RAISE NOTICE '  - AI궁금한사장: %', uid_aiboss;
  RAISE NOTICE '  - 프롬프트좀요: %', uid_promptplz;
  RAISE NOTICE '  - 쉽게설명좀: %', uid_easyplz;
...
-- 확인 쿼리 (선택사항)
-- ═══════════════════════════════════════════════════════
-- SELECT p.nickname, p.user_id, p.is_active, pr.nickname as profile_nickname
-- FROM personas p
-- JOIN profiles pr ON pr.id = p.user_id
-- WHERE p.nickname IN ('헤비업로더','인사이트호소인','프로불편러','AGI만세','ㄷㄷ형님들','AI궁금한사장','프롬프트좀요','쉽게설명좀');
