...
export async function rewriteArticle(
  ...
  // 원본 커뮤니티에서 긁어온 진짜 사람 댓글을 AI에게 컨닝 페이퍼로 주입
...
→ 위 댓글들의 말투, 길이감, 냉소적 태도, 초성체(ㅋㅋ, ㅎㅎ), 음슴체(~함, ~임)를 완벽하게 모방해서 인터넷 망령처럼 글을 써라.
→ 댓글을 베끼지 말고 "분위기와 톤"만 흡수해라.`;
  }

  // ─── 시스템 프롬프트: 극한의 사람 말투 + Few-Shot RAG v3 ───
...
[제목] 디시/블라인드 스타일. 짧고 임팩트. "~하는 거 실화?", "~인 건 나만?", "ㅋㅋㅋ" 등.
...
[최종 검수] 다 쓰고 읽어봐. "이거 AI가 쓴 거 같다" 느낌이 1%라도 나면 처음부터 다시 써.`;

  // ─── 유저 프롬프트: 원본 콘텐츠 + Few-Shot 컨텍스트 ───
  const userPrompt = `아래 기사를 참고해서 커뮤니티에 올릴 글을 써. 기사 복붙이나 요약 절대 아님. 네가 이 소식을 보고 느낀 점, 경험, 의견을 "${persona.nickname}" 말투로.
