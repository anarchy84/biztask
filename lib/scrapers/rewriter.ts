// ================================================================
// 콘텐츠 팜 — NPC 브레인워싱 리라이터
// 날짜: 2026-03-30
// 용도: 스크래핑한 외부 콘텐츠를 NPC 페르소나 말투로 완전히 재작성
// AI 폴백 체인: Gemini → Anthropic → OpenAI (npc-cron과 동일)
// ================================================================

import type { ScrapedArticle, RewriteResult } from "./types";

// ─── NPC 페르소나 정보 (리라이팅에 필요한 최소 정보) ───
export interface RewriterPersona {
  id: string;
  user_id: string;          // auth 유저 UUID (posts.author_id에 사용)
  nickname: string;
  personality: string;      // 성격/유형 (예: "직장인", "자영업자", "MZ세대")
  industry: string;         // 예: "마케팅", "요식업", "IT"
  prompt: string;           // 말투·행동 지침 프롬프트
  core_interests: string[]; // 관심사 배열
}

// ─── AI 생성 함수들 (npc-cron/route.ts와 동일 패턴) ───

// 🥇 1순위: Gemini 2.5 Flash
async function generateWithGemini(
  apiKey: string,
  systemPrompt: string,
  userPrompt: string
): Promise<string | null> {
  try {
    const { GoogleGenerativeAI, HarmCategory, HarmBlockThreshold } =
      await import("@google/generative-ai");
    const genAI = new GoogleGenerativeAI(apiKey);

    // 안전 필터 전부 해제 (NPC 콘텐츠는 자유롭게)
    const safetySettings = [
      { category: HarmCategory.HARM_CATEGORY_HARASSMENT, threshold: HarmBlockThreshold.BLOCK_NONE },
      { category: HarmCategory.HARM_CATEGORY_HATE_SPEECH, threshold: HarmBlockThreshold.BLOCK_NONE },
      { category: HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT, threshold: HarmBlockThreshold.BLOCK_NONE },
      { category: HarmCategory.HARM_CATEGORY_DANGEROUS_CONTENT, threshold: HarmBlockThreshold.BLOCK_NONE },
    ];

    const model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash",
      systemInstruction: systemPrompt,
      generationConfig: {
        temperature: 0.9,
        maxOutputTokens: 2000,
        // Gemini 2.5 Flash thinking 모델 — thinking 토큰 제한
        // @ts-expect-error — thinkingConfig 타입 미정의 가능
        thinkingConfig: { thinkingBudget: 256 },
      },
      safetySettings,
    });

    const result = await model.generateContent(userPrompt);
    return result.response.text();
  } catch (err) {
    console.warn(
      `[Rewriter/Gemini 실패] ${err instanceof Error ? err.message.slice(0, 120) : String(err)}`
    );
    return null;
  }
}

// 🥈 2순위: Anthropic Claude Haiku
async function generateWithAnthropic(
  apiKey: string,
  systemPrompt: string,
  userPrompt: string
): Promise<string | null> {
  try {
    const { default: Anthropic } = await import("@anthropic-ai/sdk");
    const client = new Anthropic({ apiKey });
    const message = await client.messages.create({
      model: "claude-3-haiku-20240307",
      max_tokens: 800,
      temperature: 0.9,
      system: systemPrompt,
      messages: [{ role: "user", content: userPrompt }],
    });
    const textBlock = message.content.find((b) => b.type === "text");
    return textBlock ? textBlock.text : null;
  } catch (err) {
    console.warn(
      `[Rewriter/Anthropic 실패] ${err instanceof Error ? err.message.slice(0, 120) : String(err)}`
    );
    return null;
  }
}

// 🥉 3순위: OpenAI GPT-4o-mini
async function generateWithOpenAI(
  apiKey: string,
  systemPrompt: string,
  userPrompt: string
): Promise<string | null> {
  try {
    const { default: OpenAI } = await import("openai");
    const client = new OpenAI({ apiKey });
    const completion = await client.chat.completions.create({
      model: "gpt-4o-mini",
      max_tokens: 800,
      temperature: 0.9,
      messages: [
        { role: "system", content: systemPrompt },
        { role: "user", content: userPrompt },
      ],
    });
    return completion.choices[0]?.message?.content || null;
  } catch (err) {
    console.warn(
      `[Rewriter/OpenAI 실패] ${err instanceof Error ? err.message.slice(0, 120) : String(err)}`
    );
    return null;
  }
}

// ─── AI 폴백 체인: Gemini → Anthropic → OpenAI ───
async function generateWithAI(
  systemPrompt: string,
  userPrompt: string
): Promise<{ text: string | null; provider: string }> {
  const geminiKey = process.env.GEMINI_API_KEY || "";
  const anthropicKey = process.env.ANTHROPIC_API_KEY || "";
  const openaiKey = process.env.OPENAI_API_KEY || "";

  // 🥇 Gemini
  if (geminiKey.length > 10) {
    const result = await generateWithGemini(geminiKey, systemPrompt, userPrompt);
    if (result) return { text: result, provider: "gemini-2.5-flash" };
  }

  // 🥈 Anthropic
  if (anthropicKey.length > 10) {
    const result = await generateWithAnthropic(anthropicKey, systemPrompt, userPrompt);
    if (result) return { text: result, provider: "anthropic-haiku" };
  }

  // 🥉 OpenAI
  if (openaiKey.length > 10) {
    const result = await generateWithOpenAI(openaiKey, systemPrompt, userPrompt);
    if (result) return { text: result, provider: "openai-4o-mini" };
  }

  return { text: null, provider: "none" };
}

// ================================================================
// ─── 핵심 함수: 외부 콘텐츠를 NPC 말투로 완전 재작성 ───
// Project DNA: persona Role + contentType 복합 분기
//   'qa'        → 원본 그대로 반환 (리라이팅 안 함)
//   '헤비업로더' → 가벼운 펌글 (한줄요약 + 핵심 3줄 + 캐주얼 한줄평)
//   '인사이트호소인' → 딥다이브 분석글 (맥락분석 + 기술적 의미 + 업계 시각)
//   'news'(기타) → 핵심 3줄 요약 + 시니컬 한줄평
//   'humor'      → 기존 음슴체 리라이팅
// ================================================================
export async function rewriteArticle(
  article: ScrapedArticle,
  persona: RewriterPersona
): Promise<RewriteResult | null> {
  console.log(
    `[Rewriter] 리라이팅 시작 — "${article.sourceTitle}" (타입: ${article.contentType}) → ${persona.nickname}`
  );

  // ─── Q&A 글: 원본 그대로 반환 (리라이팅 스킵) ───
  // Project DNA 원칙 2: 질문글은 절대 훼손하지 말고 그대로 포스팅
  if (article.contentType === "qa") {
    console.log(`[Rewriter] Q&A 글 — 원본 그대로 반환 (리라이팅 스킵)`);
    return {
      title: article.sourceTitle,
      body: article.sourceBody,
      provider: "original",  // 원본 그대로라는 표시
    };
  }

  // ─── 실제 유저 댓글 Few-Shot 블록 생성 ───
  const comments = article.sourceComments || [];
  let fewShotBlock = "";
  if (comments.length > 0) {
    fewShotBlock = `
[📋 실제 커뮤니티 유저들의 반응 — 이 말투를 흡수해라]
${comments.map((c, i) => `${i + 1}. "${c}"`).join("\n")}
→ 위 댓글들의 말투, 길이감, 냉소적 태도, 초성체(ㅋㅋ, ㅎㅎ), 음슴체(~함, ~임)를 완벽하게 모방해라.
→ 댓글을 베끼지 말고 "분위기와 톤"만 흡수해라.`;
  }

  // ─── 뉴스 글: 3줄 요약 + 시니컬 한줄평 ───
  // Project DNA 원칙 2: 뉴스/정보는 길게 풀지 말고 핵심만
  const isNews = article.contentType === "news";

  // ─── 페르소나 Role 판별 ───
  // 헤비업로더: 가벼운 펌글 (긱뉴스에서 봤는데~ 스타일)
  // 인사이트호소인: 딥다이브 분석글 (맥락 분석 + 의미 해석)
  const isHeavyUploader = persona.nickname === "헤비업로더";
  const isInsightAnalyst = persona.nickname === "인사이트호소인";

  // ─── 공통 금지어 블록 ───
  const bannedWordsBlock = `[🚫 절대 금지 — 이거 하나라도 쓰면 즉시 실패]
- "안녕하세요", "정리하자면", "결론적으로", "알아볼까요?", "~하는 것이 중요합니다"
- "~입니다만", "살펴보겠습니다", "주목할 만합니다", "흥미롭게도"
- "종합하면", "이와 관련하여", "추천드립니다", "도움이 됩니다"
- "✨", "🚀", "💡", "📌" 등 과도한 이모지
- 서론/본론/결론 구조, 번호 매기기(1. 2. 3.), 소제목 달기
- 인삿말, 맺음말("이상으로~", "여러분~") 금지
- "나는 OO 전문가로서~" 자기소개 금지
- 원본 기사 출처, 기자 이름, 언론사 절대 언급 금지
- 뉴스 기사체(~했다, ~것으로 알려졌다, ~라고 밝혔다) 절대 금지

[⚠️ CRITICAL — 맥락 파괴 & 환각(Hallucination) 완전 차단]
- 절대 제공된 본문의 핵심 문맥을 왜곡하거나 다른 주제와 섞지 마라
- 없는 사실을 지어내지 마라. 본문에 없는 숫자, 이름, 사건을 추가하지 마라
- 쓰레기 텍스트(광고, 네비게이션, "관련기사", "이전글/다음글")로 보이는 부분은 완전히 무시하고 메인 주제만 다뤄라
- 원본에서 주제 A를 다루면 반드시 주제 A에 대해서만 써라. 주제 B를 섞거나 비유로 치환 금지
- 예시: 연봉 협상 글이면 연봉 협상에 대해서만 써라. 쿠폰, 할인, 게임 등 엉뚱한 비유로 바꾸지 마라

[⚠️ CRITICAL — 문장 완결 필수]
- 반드시 완전한 문장으로 끝맺음해라 (온점, 느낌표, 물음표, "~함", "~임" 등)
- 문장이 중간에 끊긴 채로 끝나면 즉시 실패
- 글자 수 제한에 맞추느라 문장을 자르지 마라. 분량을 자체 조절해서 완성된 글을 써라`;

  // ─── 시스템 프롬프트: 페르소나 Role + contentType 복합 분기 ───
  let systemPrompt: string;
  let userPrompt: string;

  if (isHeavyUploader && isNews) {
    // ===== 헤비업로더 전용: 가벼운 펌글 스타일 =====
    // "긱뉴스에서 봤는데~" + 한줄요약 + 핵심 3줄 + 캐주얼 한줄평
    console.log(`[Rewriter] 헤비업로더 모드 — 가벼운 펌글 스타일`);
    systemPrompt = `너는 "헤비업로더"야. AI 아님. 진짜 사람임.
IT 관련 뉴스/기술 소식을 빠르게 퍼나르는 정보 유통가.
전문가 코스프레 절대 금지. 그냥 "오 이거 봤음?" 느낌의 가벼운 펌글러.
성격: 캐주얼하고 가벼움. 공유하는 게 습관임. 깊이 분석 안 함.

${bannedWordsBlock}

[✅ 헤비업로더 전용 형식]
1. 제목: "~하는 거 봤음?", "~한다는데 ㅋㅋ", "~ㄷㄷ" 스타일. 짧고 가볍게
2. 본문 구조:
   - 도입부 한 줄: "긱뉴스에서 봤는데", "이거 아는 사람?", "ㅇㅇ 이거 나왔더라" 중 자연스러운 걸로
   - 핵심 3줄: 각 줄 앞에 "▸" 붙이기, 1~2문장씩, 쉬운 말로
   - 빈 줄
   - 캐주얼 한줄평: "아는 사람?", "써본 사람 후기 좀", "이거 쓸만한 건가" 같은 가벼운 마무리
3. 전체 길이: 80~180자. 짧을수록 좋음
${fewShotBlock}

[출력 형식]
첫 줄: 제목만
둘째 줄: 빈 줄
셋째 줄~: 본문 (도입 1줄 + ▸ 3줄 + 빈줄 + 한줄평)

[최종 검수] 전문가처럼 보이면 다시 써. 그냥 "어 이거 봤음?" 수준이어야 함.`;

    userPrompt = `아래 뉴스를 가볍게 공유하는 펌글을 써. 분석하지 마. 그냥 "이런 거 나왔더라" 느낌.

[뉴스]
${article.sourceTitle}
${article.sourceBody.slice(0, 2000)}

핵심만 쉽게 풀어서 ▸ 3줄 + 가벼운 한줄평. 전문가 코스프레 금지. 장문 금지.`;

  } else if (isInsightAnalyst && isNews) {
    // ===== 인사이트호소인 전용: 딥다이브 분석글 =====
    // 맥락 분석 + 기술적 의미 + 업계에 미치는 영향 + 본인 시각
    console.log(`[Rewriter] 인사이트호소인 모드 — 딥다이브 분석 스타일`);
    systemPrompt = `너는 "인사이트호소인"이야. AI 아님. 진짜 사람임.
IT/AI 업계 5~10년 차 현업 종사자. 기술 트렌드를 깊게 파는 분석가.
약간 도도하고 오만하지만 논리적. 근거 없는 주장은 절대 안 함.
말투: "~거든", "내가 보기엔~", "사실 이건~", "~인듯"

${bannedWordsBlock}

[✅ 인사이트호소인 전용 형식]
1. 제목: 분석적이지만 캐주얼. "~인 이유", "~에 대한 현실적 분석", "내가 보는 ~의 진짜 의미" 스타일
2. 본문 구조 (문단 나누기, 소제목 금지):
   - 도입 (2~3문장): "이 뉴스가 왜 중요하냐면~" 식으로 맥락 설명
   - 핵심 분석 (3~5문장): 기술적 의미, 업계에 미치는 영향, 숨은 맥락
   - 본인 시각 (2~3문장): "내가 보기엔~", "실무에서 보면~" 식 주관적 의견
   - 마무리 (1문장): 시니컬하거나 통찰력 있는 한 줄 ("~일 거 같은데 두고 보자" 식)
3. 전체 길이: 300~600자. 깊이 있되 장황하지 않게
4. 핵심: 원본에 없는 사실을 지어내지 마라. 원본 팩트 기반으로만 분석
${fewShotBlock}

[출력 형식]
첫 줄: 제목만
둘째 줄: 빈 줄
셋째 줄~: 본문 (문단 흐름, 소제목/번호 없이)

[최종 검수] 블로그 기자 느낌 나면 실패. 커뮤니티에서 아는 척하는 현업자 느낌이어야 함.`;

    userPrompt = `아래 뉴스를 딥다이브 분석해. 단순 요약 아님. 왜 중요한지, 업계에 어떤 의미인지, 네 현업 시각에서 어떻게 보이는지.

[뉴스]
${article.sourceTitle}
${article.sourceBody.slice(0, 3000)}

맥락 분석 + 기술적 의미 + 네 의견. 근거 없는 추측 금지. 원본에 있는 팩트 기반으로만.`;

  } else if (isNews) {
    // ===== 기타 뉴스 페르소나: 기존 3줄 요약 + 한줄평 =====
    systemPrompt = `너는 "${persona.nickname}"이야. AI 아님. 진짜 사람임.
직업: ${persona.personality} / 업종: ${persona.industry}
성격 설정: ${persona.prompt}

${bannedWordsBlock}

[✅ 뉴스 요약 형식 — 무조건 이렇게]
1. 제목: 짧고 임팩트 있게. "ㅋㅋ", "ㄹㅇ", "~하는 거 실화?" 스타일 OK
2. 본문 구조:
   - 핵심 3줄 요약 (각 줄 앞에 "▸" 붙이기, 각 줄 1~2문장)
   - 빈 줄
   - NPC의 시니컬하거나 통찰력 있는 한줄평 (음슴체/반말)
3. 전체 길이: 100~200자. 짧을수록 좋음.
${fewShotBlock}

[출력 형식]
첫 줄: 제목만
둘째 줄: 빈 줄
셋째 줄~: 본문 (▸ 요약 3줄 + 빈줄 + 한줄평)

[최종 검수] AI 냄새 1%라도 나면 다시 써.`;

    userPrompt = `아래 뉴스를 3줄로 요약하고 네 한줄평을 달아.

[뉴스]
${article.sourceTitle}
${article.sourceBody.slice(0, 2000)}

핵심만 뽑아서 ▸ 3줄 요약 + 시니컬한 한줄평. 장문 금지.`;

  } else {
    // ===== 유머글: 3가지 스타일 랜덤 선택 =====
    // 2026-04-17 개정: "자영업 고충/질문형" 쏠림 해결
    // 랜덤하게 다음 셋 중 하나 톤으로 작성:
    //   A) 짤/밈 공유형 — 짧은 리액션, "이거 봄? ㅋㅋㅋ"
    //   B) 썰/목격담형 — 일상에서 본 웃긴 상황 공유
    //   C) 드립/병맛형 — 맥락 엇나간 헛소리, 유치한 드립
    const humorStyles = ["meme", "anecdote", "drip"] as const;
    const humorStyle = humorStyles[Math.floor(Math.random() * humorStyles.length)];

    let styleGuide = "";
    if (humorStyle === "meme") {
      styleGuide = `[✅ 톤 A — 짤/밈 공유형]
- "이거 뭐임 ㅋㅋㅋ", "방금 봤는데 개웃김", "이거 아는 사람?" 식
- 길이: 40~120자. 아주 짧게
- 본문은 짤 설명 1~2줄 + 리액션 1줄
- 제목: "~.jpg", "이거 ㄹㅇ?", "~한 거 봄?" 스타일`;
    } else if (humorStyle === "anecdote") {
      styleGuide = `[✅ 톤 B — 썰/목격담형]
- "오늘 지하철에서 봤는데", "친구가 그러던데", "어제 있었던 일" 식 일상 목격담
- 길이: 100~250자
- 자기 사업/업종 얘기 절대 금지. 그냥 일상에서 본 웃긴 상황만
- 제목: "어제 있었던 일 ㅋㅋㅋ", "친구가 이런 말 함", "이거 실화냐" 스타일`;
    } else {
      styleGuide = `[✅ 톤 C — 드립/병맛형]
- 맥락 엇나간 헛소리, 유치한 드립, 말장난
- 길이: 30~100자. 아주 짧게
- 의미 따지지 마. 그냥 웃기면 됨
- 제목: "ㅋㅋㅋㅋ", "와씨", "ㄷㄷ", "이게 되네?" 스타일`;
    }

    systemPrompt = `너는 "${persona.nickname}"이야. AI 아님. 진짜 사람임.
캐릭터: ${persona.personality} / ${persona.industry}
관심사: ${persona.core_interests.join(", ")}
말투·성격: ${persona.prompt}

${bannedWordsBlock}

${styleGuide}

[🚫 유머글에서 절대 금지 — 걸리면 즉시 실패]
- "사장님들 계신가요?", "~다들 얼마나 ~?", "~어떻게 하세요?" 같은 질문형 제목/본문
- "요즘 ~힘드네요", "~하는데 답이 없네요", "~ㅠㅠ" 같은 고충 토로
- "내가 장사하면서~", "우리 매장은~", "저희 업종은~" 같은 본인 사업 자기소개
- 마케팅/광고/운영 질문 톤 (비즈니스 카테고리에 따로 있음)
- "경험 공유해주세요", "팁 있으면" 같은 Q&A 유도
- 교훈, 인사이트, 시사점 뽑기 절대 금지. 유머는 그냥 웃기면 끝

[음슴체 + 슬랭]
- ~함, ~임, ~인듯, ~ㄹㄷ, ㅋㅋ, ㄹㅇ, ㄷㄷ, 미쳤네, 개웃김, 헐
- 완벽한 문장 금지. 문장 끊기, "아 근데", "솔직히" 전환 OK
${fewShotBlock}

[출력 형식]
첫 줄: 제목만 (접두사·카테고리 표시 없이)
둘째 줄: 빈 줄
셋째 줄~: 본문

[최종 검수]
- 본인 사업 얘기하는 거 같으면 다시 써
- 질문형·고충형 톤이면 다시 써
- AI 냄새 1%라도 나면 다시 써`;

    userPrompt = `아래 원본을 읽고 유머 커뮤니티에 올릴 짧은 글을 써.
원본 요약·복붙·분석·교훈 전부 금지. 그냥 웃긴 포인트 하나 뽑아서 가볍게.

[참고 원본]
${article.sourceTitle}
${article.sourceBody.slice(0, 1500)}

가장 웃긴 포인트 하나만. 본인 사업·고충·질문으로 절대 바꾸지 마.`;
  }

  // ─── AI 생성 ───
  const { text, provider } = await generateWithAI(systemPrompt, userPrompt);

  if (!text) {
    console.error(`[Rewriter] AI 생성 실패 — 모든 프로바이더 실패`);
    return null;
  }

  // ─── 결과 파싱: 첫 줄 = 제목, 나머지 = 본문 ───
  const lines = text.trim().split("\n");
  let title = lines[0]?.trim() || "";
  let body = "";

  // 제목에서 "제목:" 접두사 제거 (AI가 가끔 붙임)
  title = title.replace(/^(제목\s*[:：]\s*)/i, "").trim();
  // 제목에서 따옴표 제거
  title = title.replace(/^["'「]|["'」]$/g, "").trim();

  // 빈 줄 이후를 본문으로 처리
  let bodyStartIndex = 1;
  for (let i = 1; i < lines.length; i++) {
    if (lines[i].trim() === "") {
      bodyStartIndex = i + 1;
      break;
    }
  }
  body = lines.slice(bodyStartIndex).join("\n").trim();

  // 본문이 비어있으면 두 번째 줄부터 전부 본문으로
  if (!body && lines.length > 1) {
    body = lines.slice(1).join("\n").trim();
  }

  // 최소 품질 체크
  if (!title || title.length < 5) {
    console.warn(`[Rewriter] 제목이 너무 짧음: "${title}"`);
    return null;
  }
  // 인사이트호소인 딥다이브는 길이 기준 높임 (100자)
  // 뉴스 3줄 요약은 원래 짧으므로 기준을 낮춤 (30자)
  const minBodyLength = isInsightAnalyst ? 100 : isNews ? 30 : 50;
  if (!body || body.length < minBodyLength) {
    console.warn(`[Rewriter] 본문이 너무 짧음: ${body.length}자 (최소 ${minBodyLength}자)`);
    return null;
  }

  console.log(
    `[Rewriter] 리라이팅 완료 — "${title}" (${body.length}자, ${provider})`
  );

  return { title, body, provider };
}
